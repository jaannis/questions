<!DOCTYPE html>

<html>
    <head>
        <title></title>
        <link href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
        <link href="/style/main.css" rel="stylesheet" type="text/css"/>
    </head>
    <body>
        
  
     <form action="/questions" method="POST">
        <div class="container pagination-centered text-center">
            <div class="row">
            <div class="col-md-4">
                <h2 class="title">Testa uzdevums</h2>
                <input class="form-control" id="price" type="text" name="price"
                placeholder="Ievadi savu vārdu" required>
                <select class="form-control" name="type" id="typeSwitcher" >
                        <option disabled selected value> -- Izvēlies testu -- </option>
                        <?php foreach ($questions as $question) : ?>
                        <option name=<?=$question->idtest; ?> ><?=$question->name; ?></option>  
                        <?php endforeach; ?>;                                                   
                </select>
                <button class="btn btn-primary" role="button" type="submit" name="save" > Sākt </button>
            </div>                      
            </div>
        </div>
     </div>
    </form>
         

    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    <script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
    <script src="/style/main.js" type="text/javascript"></script>
    </div></body>
    </html>